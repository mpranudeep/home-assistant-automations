...
const LovUtils = {
  ...
  isDataProvider: function (data) {
    return data && DataProviderFeatureChecker
      ? DataProviderFeatureChecker.isDataProvider(data)
      : false;
  },
  ...
  isTreeDataProvider: function (data) {
    return data && DataProviderFeatureChecker
      ? DataProviderFeatureChecker.isTreeDataProvider(data)
      : false;
  }
};
...
LovDropdown.prototype._ClearSelection = function () {
  ...
  this._SetCollectionSelectedKeySet(new KeySetImpl([]));
};
...
LovDropdown.prototype._defaultCollectionRenderer = function (context) {
  ...
  if (!this._resultsElem) {
    ...
    busyContext.whenReady().then(
      ...
      function (reason) {
        warn('Select: creating default listView busyContext promise rejected: ' + reason);
        this._clearContextRenderPromiseFunctions(context);
        contextRenderError(reason);
      }.bind(this)
    );
...
oj.__registerWidget('oj.ojSelectBase', $.oj.editableValue, {
  ...
  _wrapDataProviderIfNeeded: function (dataProvider) {
    if (LovUtils.isDataProvider(dataProvider)) {
      ...
      wrapper = getEnhancedDataProvider(wrapper, {
...
export { AbstractLovBase, FilteringDataProviderView, LovDropdown, LovMainField, LovUtils };
